...
//				mIsDm365 = true;
				Log.w(THIS_FILE, "isDm365:" + mIsDm365
				...
				if (pjService.mediaManager != null) {
					if (pjService.service.getGSMCallState() == TelephonyManager.CALL_STATE_IDLE
						...
						pjService.mediaManager
								.playInCallTone(MediaManager.TONE_CALL_WAITING);
					}
				}
		...
		if (mIsDm365) {
			try {
				final SipCallSession callInfo = updateCallInfoFromStack(callId,
						e);
				android.util.Log.i("aaa","aaa getCallState:"+callInfo.getCallState());
	...
	public void on_buddy_state(int buddyId) {
		...
		pjService.service.presenceMgr.changeBuddyState(
				PjSipService.pjStrToString(binfo.getUri()),
	...
	public void on_pager_status(int callId, pj_str_t to, pj_str_t body,
		...
		cr.update(SipMessage.MESSAGE_URI, args, SipMessage.FIELD_TO + "=? AND "
				+ SipMessage.FIELD_BODY + "=? AND " + SipMessage.FIELD_TYPE
				+ "=" + SipMessage.MESSAGE_TYPE_QUEUED, new String[] { toStr,
				bodyStr });
		...
		public void handleMessage(Message msg) {
			...
			case ON_CALL_STATE: {
				...
				case SipCallSession.InvState.CALLING:
					stateReceiver.notificationManager
				...
				case SipCallSession.InvState.DISCONNECTED:
					...
					stateReceiver.pjService.service.getContentResolver()
							.insert(SipManager.CALLLOG_URI, cv);
					Integer isNew = cv.getAsInteger(CallLog.Calls.NEW);
					if (isNew != null && isNew == 1) {
						stateReceiver.notificationManager
								.showNotificationForMissedCall(cv);
					}
					...
					if (stateReceiver.mIntegrateWithCallLogs) {
						...
						ParsedSipContactInfos callerInfos = SipUri
								.parseSipContact(cv.getAsString(Calls.NUMBER));
						if (callerInfos != null) {
							String phoneNumber = SipUri
									.getPhoneNumber(callerInfos);
							...
							if (!TextUtils.isEmpty(phoneNumber)) {
								...
								if (callInfo.getAccId() != SipProfile.INVALID_ID) {
									...
									if (acc != null && acc.display_name != null) {
										extraCv.put(
												CallLogHelper.EXTRA_SIP_PROVIDER,
												acc.display_name);
									}
								}
								CallLogHelper.addCallLog(
										stateReceiver.pjService.service, cv,
										extraCv);
							}
						}
					}
	...
	private void broadCastAndroidCallState(String state, String number) {
		...
		if (!Compatibility.isCompatible(19)) {
			...
			intent.putExtra(pjService.service.getString(R.string.app_name),
					true);
	...
	private synchronized void launchCallHandler(SipCallSession currentCallInfo2) {
		...
		if (lastLaunchCallHandler + LAUNCH_TRIGGER_DELAY < currentElapsedTime) {
			...
			Intent callHandlerIntent = SipService.buildCallUiIntent(ctxt,
					currentCallInfo2);
