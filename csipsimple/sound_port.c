...
//pj_bool_t pjmedia_audio_use_speex_ns = PJ_TRUE;
pj_bool_t pjmedia_audio_use_speex_ns = PJ_FALSE;
...
//	PJ_LOG(5,(THIS_FILE, "speex noise suppress"));
				       ...
				       pjmedia_snd_port *snd_port )
{
	...
	return status;
		...
		PJ_LOG(5,(THIS_FILE, "samples_per_frame:%d,clock_rate%d",snd_port->samples_per_frame,snd_port->clock_rate));
		
		#if 1
		if (pjmedia_audio_use_speex_ns == PJ_TRUE){
			if (snd_port->speex_st){
		PJ_LOG(5,(THIS_FILE, "speex noise suppress destroy"));
		speex_preprocess_state_destroy(snd_port->speex_st);
			}
			PJ_LOG(5,(THIS_FILE, "speex noise suppress start"));
