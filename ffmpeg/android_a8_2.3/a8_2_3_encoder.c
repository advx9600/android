...
int encoder_init(A8MfcEncParam* param)
{
	...
	switch(param->codecType)
	{
		case H264_ENC:
			...
			param->param264.IDRPeriod = 1; // gop
	...
	param->mfcHandle = (_MFCLIB*) SsbSipMfcEncOpen();
	...
	if ((ret = SsbSipMfcEncGetOutBuf(param->mfcHandle, \
		&param->outputInfo)) != MFC_RET_OK) {
	...
	memcpy(param->headerData, param->outputInfo.StrmVirAddr, \
		param->headerSize);

	if (SsbSipMfcEncGetInBuf(param->mfcHandle, &param->inputInfo) != MFC_RET_OK) {
...
int encoder_exe(A8MfcEncParam* param, char* buf)
{
	...
	if (buf == NULL) buf=param->inputData;
	else memcpy(param->inputData,buf,param->width*param->height*3/2);

	memcpy(param->inputInfo.YVirAddr, param->inputData, param->width * param->height);
...
{
	if (param->headerData != NULL) free(param->headerData);
